...
- Una sola herramienta para reemplazar `pip`, `pip-tools`, `pipx`, `poetry`, `pyenv`, `twine`,
  `virtualenv` y más.
...
- Incluye una [interfaz compatible con pip](#the-pip-interface) para obtener una mejora de
  rendimiento con una CLI familiar.
- Admite [workspaces](https://docs.astral.sh/uv/concepts/projects/workspaces) al estilo de Cargo
  para proyectos escalables.
- Eficiente en espacio de disco, con una
  [caché global](https://docs.astral.sh/uv/concepts/cache) para deduplicar dependencias.
- Puede instalarse sin Rust ni Python mediante `curl` o `pip`.
...
Consulta la [documentación de instalación](https://docs.astral.sh/uv/getting-started/installation/)
para obtener detalles y métodos de instalación alternativos.

## Documentación

La documentación de uv se encuentra en [docs.astral.sh/uv](https://docs.astral.sh/uv).
...
uv también admite compilar y publicar proyectos, incluso si no se administran con uv. Consulta la
...
Descarga versiones de Python cuando sea necesario:
...
resoluciones independientes de la plataforma, resoluciones reproducibles, estrategias de resolución
alternativas y más.

Migra a uv sin cambiar los flujos de trabajo existentes — y experimenta una aceleración de 10-100x
— con la interfaz `uv pip`.
...
uv apoya a personas colaboradoras de todos los niveles de experiencia y valora su participación en
el proyecto. Consulta la
...
Solo "uv", por favor. Consulta la [guía de estilo](./STYLE.md#styling-uv) para obtener detalles.

#### ¿Qué plataformas admite uv?

Consulta el documento de [compatibilidad de plataformas de
uv](https://docs.astral.sh/uv/reference/platforms/).

#### ¿Está uv listo para producción?

Sí, uv es estable y se usa ampliamente en producción. Consulta el documento de
[política de versiones de uv](https://docs.astral.sh/uv/reference/versioning/) para obtener detalles.

## Agradecimientos

El resolvedor de dependencias de uv utiliza [PubGrub](https://github.com/pubgrub-rs/pubgrub) como
base. uv agradece a quienes mantienen PubGrub, en especial a
[Jacob Finkelman](https://github.com/Eh2406), por su apoyo.

La implementación de Git de uv se basa en [Cargo](https://github.com/rust-lang/cargo).

Algunas de las optimizaciones de uv se inspiran en el gran trabajo de
[pnpm](https://pnpm.io/), [Orogene](https://github.com/orogene/orogene) y
[Bun](https://github.com/oven-sh/bun). uv también tomó aprendizajes del proyecto
[Posy](https://github.com/njsmith/posy) de Nathaniel J. Smith y adaptó su
[trampolín](https://github.com/njsmith/posy/tree/main/src/trampolines/windows-trampolines/posy-trampoline)
para el soporte en Windows.
...
Salvo que se indique explícitamente lo contrario, cualquier contribución enviada de forma
intencionada para su inclusión en uv, según la definición de la licencia Apache-2.0, quedará bajo
doble licencia como se indica arriba, sin términos ni condiciones adicionales.
