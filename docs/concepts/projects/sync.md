...
- `UV_NO_INSTALL_LOCAL=1`: Skip the current project, workspace members, and other local path or editable packages, and install only remote dependencies
...
These environment variables provide the same partial installation behavior when `uv` runs through wrappers or environments that do not make command line flags easy to pass through. If the matching hidden `--only-install-*` mode is used, it overrides the corresponding `UV_NO_INSTALL_*` setting instead of combining with it.
