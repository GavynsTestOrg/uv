...
uv will skip creating a Python version pin file, a README, version control files and directories,
and any source directories or files.
...
For example, `--vcs git` adds version control files and directories, and `--python-pin` adds a
Python version pin file.
